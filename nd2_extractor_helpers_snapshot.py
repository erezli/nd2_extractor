...
# Modified for extracting from a number of snapshot grids (with only one time point, but more than one FOV) by Charlie Wedd on 27 April 2022
#TODO make robust to extract from single timepoint grids of any size (e.g. 1 FOV or more in any layout)
#TODO (possibly) remove manual checks of number of FOVs, timepoints etc and assume it just works so that there is no need for manual input in the looping
...
def predict_t(directory): 
    ...
    # in case where nd2 is a single image (no grid, no time series, time points = 1)
    if frames.metadata["sequence_count"] == 1:
        return 1
...
def predict_FOVs(directory):
    ...
    # in case where nd2 is a single image (no grid, no time series, FOVs = 1)
    if frames.metadata["sequence_count"] == 1:
        return 1
    ...
    try:
        ...
        if predict_t(directory) == 1:
            return int(frames.metadata["sequence_count"])
        else:
            print("Something else went wrong with the FOV prediction")
    frames.close()
